...
namespace Corvalius.Membership.Raven
{
    ...
    public static class OAuthWebSecurity
    {
        ...
        /// <param name="appId">The consumer key.</param>
        /// <param name="appSecret">The consumer secret.</param>
        public static void RegisterOAuthClient(BuiltInOAuthClient client, string consumerKey, string consumerSecret)
        {
            ...
            switch (client)
            {
                case BuiltInOAuthClient.LinkedIn:
                    authenticationClient = new LinkedInCustomClient(consumerKey, consumerSecret);
                    break;

                case BuiltInOAuthClient.Twitter:
                    authenticationClient = new TwitterCustomClient(consumerKey, consumerSecret);
                    break;
                ...
                case BuiltInOAuthClient.WindowsLive:
                    authenticationClient = new MicrosoftClient(consumerKey, consumerSecret);
                    break;

                case BuiltInOAuthClient.Github:
                    authenticationClient = new GithubClient(consumerKey, consumerSecret);
                    break;
        ...
        public static IEnumerable<IAuthenticationClient> GetRegisteredClients()
        {
            foreach (var item in _authenticationClients)
            {
                yield return item;
            }
        }
        ...
        public static void RegisterClient(IAuthenticationClient client)
        {
            ...
            {
                throw new ArgumentException(Resources.InvalidServiceProviderName, "client");
            }

            if (_authenticationClients.Contains(client))
            {
                throw new ArgumentException(Resources.ServiceProviderNameExists, "client");
            }
        ...
        internal static AuthenticationResult VerifyAuthenticationCore(HttpContextBase context, string returnUrl)
        {
            ...
            if (TryGetOAuthClient(providerName, out client))
            {
                ...
                // TODO: Check this as it is a very different way in 4.2 instead of the original 4.0 used by Microsoft.
                return securityManager.VerifyAuthentication(returnUrl);
            }
            else
            {
                throw new InvalidOperationException(Resources.InvalidServiceProviderName);
            }
        }
        ...
        internal static bool GetIsAuthenticatedWithOAuthCore(HttpContextBase context)
        {
            ...
            {
                var securityManager = new OpenAuthSecurityManager(context, client, OAuthDataProvider);
                return securityManager.IsAuthenticatedWithOpenAuth;
            }
            else
            {
                throw new InvalidOperationException(Resources.InvalidServiceProviderName);
        ...
        public static ICollection<OAuthAccount> GetAccountsFromUserName(string userName)
        {
            if (String.IsNullOrEmpty(userName))
            {
                throw new ArgumentException(
                    String.Format(CultureInfo.CurrentCulture, Resources.Argument_Cannot_Be_Null_Or_Empty, "userName"),
                    "userName");
            }
        ...
        internal static IAuthenticationClient GetOAuthClient(string providerName)
        {
            ...
            {
                throw new ArgumentException(Resources.ServiceProviderNotFound, "providerName");
            }
